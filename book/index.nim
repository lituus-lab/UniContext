...
For C, the build produces `libUniContext.a` and `include/UniContext.h`:

```bash
build/unigate clibStatic
cc -Iinclude your.c libUniContext.a
```

The PyPI distribution is `lituus-unicontext`; the import name stays
`unicontext`. Those are two decisions, and the bare names are not all
available.
...
nbCode:
  ...
  echo "version ", UniContextVersion
