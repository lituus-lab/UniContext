...
const
  sharedLib =
    when defined(windows): "libUniContext.dll"
    elif defined(macosx): "libUniContext.dylib"
    else: "libUniContext.so"
  staticLib = "libUniContext.a"  # MinGW `ar` on Windows, so `.a` everywhere.
...
task clibMsvc, "C static library, MSVC ABI (Windows Python extension)":
  ...
  exec "nim c --path:src --cc:vcc --app:staticlib --noMain --mm:arc -d:release -d:noAutoInit" &
       " -o:UniContext.lib src/UniContext/c_api.nim"
  done "clibMsvc"
