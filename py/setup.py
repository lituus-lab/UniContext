...
"""Build unicontext._core, a Cython extension over the UniContext C ABI.
...
PKG_DIR = os.path.join(HERE, "unicontext")
...
if sys.platform == "win32":
    LIB_NAME, BUNDLED = "UniContext.lib", False
    LINK_ARGS, NIMBLE_TASK = [], "clibMsvc"
elif sys.platform == "darwin":
    LIB_NAME, BUNDLED = "libUniContext.dylib", True
    LINK_ARGS, NIMBLE_TASK = ["-Wl,-rpath,@loader_path"], "clib"
else:
    LIB_NAME, BUNDLED = "libUniContext.so", True
...
def nim_project_dir():
    """Where UniContext.nimble lives: the real repo root in a normal
...
def ensure_lib_built():
    ...
    try:
        ...
        raise SystemExit(
            "setup.py: `nimble` not found on PATH. Building unicontext from "
...
# The sdist ships the pre-transpiled unicontext/_core.c, not the .pyx (Cython
...
ext = Extension(
    "unicontext._core",
    sources=[pyx if os.path.exists(os.path.join(HERE, pyx)) else os.path.join("unicontext", "_core.c")],
    include_dirs=[INCLUDE],
    library_dirs=[LIB_DIR],
    extra_link_args=LINK_ARGS,
    libraries=["UniContext"],
)
...
setup(
    ...
    package_data={"unicontext": [LIB_NAME] if BUNDLED else []},
    exclude_package_data={"unicontext": ["_core.c"]},
    zip_safe=False,
)
