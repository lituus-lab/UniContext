...
## C ABI for UniContext. Built --app:staticlib/--app:lib --noMain --mm:arc
## -d:release. Keep in sync with include/UniContext.h; tests/c links the header
...
const UniContextVersionC: cstring = "0.1.0"
...
#ifdef _WIN32
#  include <windows.h>
static INIT_ONCE unicontext_runtime_once = INIT_ONCE_STATIC_INIT;
static BOOL CALLBACK unicontext_runtime_init(PINIT_ONCE o, PVOID p, PVOID *c) {
  (void)o; (void)p; (void)c; NimMain(); return TRUE;
}
static void unicontext_runtime_ensure(void) {
  InitOnceExecuteOnce(&unicontext_runtime_once, unicontext_runtime_init, NULL, NULL);
}
#else
#  include <pthread.h>
static pthread_once_t unicontext_runtime_once = PTHREAD_ONCE_INIT;
static void unicontext_runtime_init(void) { NimMain(); }
static void unicontext_runtime_ensure(void) {
  pthread_once(&unicontext_runtime_once, unicontext_runtime_init);
}
#endif
""".}
  template ensureRuntime() =
    {.emit: "  unicontext_runtime_ensure();".}
else:
  template ensureRuntime() = discard

{.push exportc, cdecl, dynlib, raises: [].}

proc unicontext_fibonacci(n: cint): clonglong =
